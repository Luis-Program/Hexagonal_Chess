...
public class Bishop extends Piece {
    ...
    public Bishop(int posX, int posY, String name, String color, JPanel baseChess, chessHex ch) {
    ...
    public boolean Check(int x, int y) {
        ...
        if (this.getCh().getChess().containsKey(y) && this.getCh().getChess().get(y).containsKey(x)) {
            if (this.getCh().getChess().get(y).get(x).isOccuped()) {
                ...
                && (this.gethY() == y)) {
                    return  this.IsOccuped(this.gethX(), this.gethY(), x, y); //metodo
                }else {
                    for (int i = 1; i < 5; i++) {
                        if ((!this.getColor().equals(this.getCh().getChess().get(y).get(x).getColor()))&& 
    ...
    public boolean Move(int x, int y) {
      ...
      public boolean March(int alterX, int alterY, int cont ,int d){
          int x = alterX;
          int y = alterY;
